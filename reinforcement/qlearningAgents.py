...
class QLearningAgent(ReinforcementAgent):
    ...
    def computeValueFromQValues(self, state):
        ...
        for actions in self.getLegalActions(state):
        	qValue = self.getQValue(state, actions)
        	if qValue > maxValue:
        		maxValue = qValue
        if maxValue == -9999999999:
        	return 0.0
        else:
        	return maxValue
    ...
    def update(self, state, action, nextState, reward):
        ...
        learned = self.alpha * (reward + discountedValue)
        notLearned = (1 - self.alpha) * qVal
        self.qValues[(state, action)] = learned + notLearned
...
class ApproximateQAgent(PacmanQAgent):
    ...
    def getQValue(self, state, action):
        ...
        for feature, val in features.iteritems():
        	total += self.getWeights()[feature] * val
        return total
    ...
    def update(self, state, action, nextState, reward):
        ...
        discountedValue = self.discount * self.getValue(nextState)
        newValue = discountedValue + reward
        difference = newValue - qVal
        features = self.featExtractor.getFeatures(state, action)
        for feature, value in features.iteritems():
        	weight = self.alpha * difference * features[feature]
        	self.getWeights()[feature] += weight
