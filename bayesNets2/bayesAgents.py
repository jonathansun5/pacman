...
def constructBayesNet(gameState):
    ...
    "*** YOUR CODE HERE ***"
    # populating `obsVars` using the procedure above
    for housePos in gameState.getPossibleHouses():
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
    ...
    # populating `edges` with every edge in the Bayes Net (a tuple `(from, to)`)
    ...
    # setting each `variableDomainsDict[var] = values`, where `values` is the set of possible assignments to `var`. These should again be set using the constants defined at the top of this file.
...
def fillObsCPT(bayesNet, gameState):
    ...
    for housePos in gameState.getPossibleHouses():
        # looking at each wall of that house
        for obsPos in gameState.getHouseWalls(housePos):
            obsVar = OBS_VAR_TEMPLATE % obsPos
            oVFactor = bn.Factor([obsVar], [GHOST_HOUSE_VAR, FOOD_HOUSE_VAR], bayesNet.variableDomainsDict())
            # looking at each assignment (wall color, ghost house position, food house position) of that wall
            for assignment in oVFactor.getAllPossibleAssignmentDicts():
                # determine location of house and assign its location value
                unknownHouse = None
                maxDist = 9999999999
                for house in gameState.getPossibleHouses():
                    if util.manhattanDistance(obsPos, house) < maxDist:
                        maxDist = util.manhattanDistance(obsPos, house)
                        unknownHouse = house
                if unknownHouse == bottomLeftPos:
                    location = BOTTOM_LEFT_VAL
                elif unknownHouse == topLeftPos:
                    location = TOP_LEFT_VAL
                elif unknownHouse == bottomRightPos:
                    location = BOTTOM_RIGHT_VAL
                else:
                    location = TOP_RIGHT_VAL
                # assign probabilities given color type (red, blue, or none) and the location and type of adjacent house center
                probability = 0
                # red observation
                if assignment[obsVar] == RED_OBS_VAL:
                    if location == assignment[FOOD_HOUSE_VAR]:
                        probability = PROB_FOOD_RED
                    elif location == assignment[GHOST_HOUSE_VAR]:
                ...
                if assignment[obsVar] == NO_OBS_VAL:
                    if location == assignment[FOOD_HOUSE_VAR]:
                        ...
                        probability = 1
                oVFactor.setProbability(assignment, probability)
            bayesNet.setCPT(obsVar, oVFactor)
...
class VPIAgent(BayesAgent):
    ...
    def computeEnterValues(self, evidence, eliminationOrder):
        ...
        *Do not* take into account the "time elapsed" cost of traveling to each
        ...
        probabilityFoodTopLeft = 0
        probabilityFoodTopRight = 0
        for allPossibleAssignmentDicts in factor.getAllPossibleAssignmentDicts():
            # calculate probability of FOOD_HOUSE_VAR at TOP_LEFT_VAL and GHOST_HOUSE_VAR at TOP_RIGHT_VAL given evidence
            if allPossibleAssignmentDicts[FOOD_HOUSE_VAR] == TOP_LEFT_VAL and allPossibleAssignmentDicts[GHOST_HOUSE_VAR] == TOP_RIGHT_VAL:
                probabilityFoodTopLeft = factor.getProbability(allPossibleAssignmentDicts)
            # calculate probability of FOOD_HOUSE_VAR at TOP_RIGHT_VAL and GHOST_HOUSE_VAR at TOP_LEFT_VAL given evidence
            if allPossibleAssignmentDicts[FOOD_HOUSE_VAR] == TOP_RIGHT_VAL and allPossibleAssignmentDicts[GHOST_HOUSE_VAR] == TOP_LEFT_VAL:
                probabilityFoodTopRight = factor.getProbability(allPossibleAssignmentDicts)
        leftExpectedValue = (probabilityFoodTopLeft * WON_GAME_REWARD) + ((1  - probabilityFoodTopLeft) * GHOST_COLLISION_REWARD)
        rightExpectedValue = (probabilityFoodTopRight) * WON_GAME_REWARD + ((1 - probabilityFoodTopRight) * GHOST_COLLISION_REWARD)
    ...
    def computeExploreValue(self, evidence, enterEliminationOrder):
        ...
        for prob, explorationEvidence in self.getExplorationProbsAndOutcomes(evidence):
            evidence.update(explorationEvidence)
            enteringValue = max(self.computeEnterValues(evidence, enterEliminationOrder))
            expectedValue = expectedValue + (prob * enteringValue)
