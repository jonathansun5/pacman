...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet, queryVariables, evidenceDict, eliminationOrder):
        ...
        for item in eliminationOrder:
            allCPTs, joinFactor = joinFactorsByVariable(allCPTs, item)
            if len(joinFactor.unconditionedVariables()) > 1:
                eliminated = eliminate(joinFactor, item)
                allCPTs.append(eliminated)
        final_factor = joinFactors(allCPTs)
